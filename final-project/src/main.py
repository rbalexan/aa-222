...
from active_learning import variance_based_active_learning, lola_active_learning
from benchmark_functions import sinc, hebbal, step
from metrics import compute_ise_and_iv, compute_ise
from models import fit_gaussian_process, fit_neural_network
from plot_functions import plot_model, plot_ise_and_iv, plot_kernel_comparison
...
if __name__ == "__main__":
    ...
    surrogate_models = ['gp']
    active_learning_methods = [variance_based_active_learning, lola_active_learning]
    active_learning_flags   = ['variance', 'lola']

    # gp parameters
    kernels = [ConstantKernel(), RBF(), Matern(nu=1/2), Matern(nu=3/2), Matern(nu=5/2), RationalQuadratic(), ExpSineSquared(), DotProduct()]
    kernel_flags = ['constant', 'rbf', 'matern12', 'matern32', 'matern52', 'rationalquad', 'expsinesqr', 'dot']
    kernel_labels = ['Constant', 'RBF', 'Matérn-1/2', 'Matérn-3/2', 'Matérn-5/2', 'RQ',
                    'ESS', 'Dot Prod.']

    # nn parameters
    activations                   = ['relu', 'leakyrelu', 'selu']
    hidden_units                  = [64]
    epochs                        = 10000
    stop_early_callback           = keras.callbacks.EarlyStopping(monitor='mse', min_delta=1e-6, patience=1000, verbose=0, restore_best_weights=True)
    ...
    for fs, surrogate_model in enumerate(surrogate_models):                    # loop over surrogate model types
        ...
        for fa, active_learning_method in enumerate(active_learning_methods):  # loop over active learning methods
                ...
                for fk, kernel in enumerate(kernels):                          # iterate over kernels
                    ...
                    for n in range(n_random_trials):
                        ...
                        for k in range(k_max + 1):
                            ...
                            x_train_new, y_train_new = active_learning_method(gp, f, X_pred, std_pred, X_train)
            ...
            if surrogate_model == 'nn' and flags[1] == 'variance':
                continue
            # for nns, iterate over activation functions
            elif surrogate_model == 'nn':

                ise_activation = np.zeros((len(activations), n_random_trials, k_max))
                ...
                for fk, activation in enumerate(activations):
                    ...
                    for n in range(n_random_trials):
                        ...
                        for k in range(k_max + 1):
                            ...
                            # get new sample using active learning method
                            x_train_new, y_train_new = active_learning_method(nn, f, X_pred, [], X_train)
