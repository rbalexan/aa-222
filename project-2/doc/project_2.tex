...
\begin{figure*}[h]
    \centering
    \includesvg[width=0.49\linewidth]{../plots/simple2_fconv_alg1.svg}
    \includesvg[width=0.49\linewidth]{../plots/simple2_cconv_alg1.svg}
    \caption{Convergence of objective function (\textit{left}) and the maximal constraint violation (\textit(right)) for three randomly-initialized constrained optimizations on the \texttt{simple2} function. For the exterior method, these optimizations use the quadratic penalty method ($\rho=1$, $\gamma=2$) with gradient descent with Nesterov momentum ($\alpha=1\textsc{E}-3$, $\beta=0.9$) and for the interior method, these optimizations use the inverse-barrier interior point method ($\rho=1$, $\gamma=2$) on top of the Hooke-Jeeves method ($\alpha_0=1\textsc{E}-1$, $\gamma=0.5$, $\epsilon=1\textsc{E}-4$, $k_{max}=10$). The color gradations for \texttt{simple2} are the base-10 logarithm of the function values.}
    \label{fig:alg1_conv}
\end{figure*}

\begin{figure*}[h]
    \centering
    \includesvg[width=0.49\linewidth]{../plots/simple2_fconv_alg2.svg}
    \includesvg[width=0.49\linewidth]{../plots/simple2_cconv_alg2.svg}
    \caption{Convergence of objective function (\textit{left}) and the maximal constraint violation (\textit(right)) for three randomly-initialized constrained optimizations on the \texttt{simple2} function. For the exterior method, these optimizations use the mixed penalty method ($\rho_1=1$, $\rho_2=1$, $\gamma=2$) with gradient descent with Nesterov momentum ($\alpha=1\textsc{E}-3$, $\beta=0.9$) and for the interior method, these optimizations use the log-barrier interior point method ($\rho=1$, $\gamma=2$) on top of the Hooke-Jeeves method ($\alpha_0=1\textsc{E}-1$, $\gamma=0.5$, $\epsilon=1\textsc{E}-4$, $k_{max}=10$).}
    \label{fig:alg2_conv}
\end{figure*}
